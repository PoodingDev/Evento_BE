...
class FavoriteEventApiTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.event = Event.objects.create(
            title="Test Event", start_time="2024-12-01T10:00:00Z"
        )
    ...
    def test_create_favorite_event(self):
        ...
        """
        url = reverse(
            "favorites:favorite-event-add", args=[self.event.id]
        )  # 'favorite-event-add'를 사용
    ...
    def test_add_favorite_event_already_exists(self):
        ...
        url = reverse(
            "favorite:favorite-event-add", args=[self.event.id]
        )  # 'favorite-event-add'를 사용
        response = self.client.post(url)
