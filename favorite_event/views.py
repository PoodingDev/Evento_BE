...
from calendars.models import Event  # 이벤트 모델을 가져옵니다.
...
class FavoriteEventDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = FavoriteEvent.objects.all()  # 특정 즐겨찾기 이벤트를 가져오기 위한 쿼리셋
    serializer_class = FavoriteEventSerializer  # 데이터를 직렬화할 때 사용할 시리얼라이저
...
class FavoriteEventAdd(APIView):
    ...
    def post(self, request, event_id):
        ...
        if FavoriteEvent.objects.filter(user=request.user, event=event).exists():
            return Response({"detail": "이미 즐겨찾기한 이벤트입니다."}, status=status.HTTP_400_BAD_REQUEST)

        # 즐겨찾기 이벤트 추가
        FavoriteEvent.objects.create(user=request.user, event=event)

        return Response({"detail": "즐겨찾기 이벤트가 추가되었습니다."}, status=status.HTTP_201_CREATED)


class FavoriteEventAdd(APIView):
    def post(self, request, event_id):
        # 이벤트 즐겨찾기 추가 로직
        return Response({"message": "Favorite event added"}, status=status.HTTP_201_CREATED)
